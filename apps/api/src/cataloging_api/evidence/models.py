...
class CatalogEvidenceSource(Base):
    ...
    __table_args__ = (Index("ix_evidence_sources_session_created", "session_id", "created_at"),)

    source_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    session_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("catalog_evidence_sessions.session_id", ondelete="CASCADE"), index=True
    )
...
class CatalogEvidenceCandidate(Base):
    ...
    source_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("catalog_evidence_sources.source_id", ondelete="CASCADE"), index=True
    )
