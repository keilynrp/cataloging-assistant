    ...
    <div className="mx-auto max-w-6xl px-6 py-10">
      ...
      <header className="mt-6 max-w-3xl">
        ...
        <p className="mt-3 text-muted">
          Cobertura y relaciones observadas en {profile.collection_name}. Esta vista ayuda a decidir
          reglas; no convierte los datos históricos en vocabulario autorizado.
        </p>
      ...
      <section className="mt-10" aria-labelledby="coverage-heading">
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-sm text-muted">Denominador: {profile.active_items} ítems activos</span>
        </div>
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {profile.fields.map((field) => (
            <article key={field.field} className="rounded-xl border border-line bg-surface p-5 shadow-sm">
      ...
      <section className="mt-10" aria-labelledby="values-heading">
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-sm text-muted">Porcentaje sobre {profile.active_items} ítems activos</span>
        </div>
      ...
      <section className="mt-10" aria-labelledby="patterns-heading">
        <div className="flex items-baseline justify-between">
          ...
          <span className="text-sm text-muted">Combinaciones de presencia, no validación semántica</span>
        </div>
                  ...
                  <tbody className="divide-y divide-line">
                    {relationship.pairs.slice(0, 10).map((pair) => (
                      <tr key={`${pair.from_value}-${pair.to_value}`}>
                        ...
                        <td className="px-2 py-2 text-muted" aria-hidden="true">→</td>
                        <td className="px-2 py-2 text-ink">{pair.to_value}</td>
                        <td className="px-5 py-2 text-right font-semibold text-ink">{pair.item_count}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
